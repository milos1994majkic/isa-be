...
public interface RatingRepository extends JpaRepository<Rating, Long> {
    ...
    @Query("SELECT COUNT(r) > 0 FROM Rating r WHERE r.user.id = :userId AND (r.doctor.id = :doctorId OR r.clinic.id = :clinicId)")
    boolean existsRatingByUserAndDoctorOrClinic(@Param("userId") long userId, @Param("doctorId") long doctorId, @Param("clinicId") long clinicId);
